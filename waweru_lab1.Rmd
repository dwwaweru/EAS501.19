...
```{r Download Libraries, results='hide'}
library(ggplot2)
library(sf)
library(tidyverse)
library(dplyr)
...
```{r Final map, fig.cap = "Map 1. This map shows the distribution of the different types of images, those close to water features or close to landmarks. It is interesting to see the clustering of the different types of images which indicates where certain features are located within Boulder Park. The map also shows that there are few water features within the study area. This is likely the most influential factor in the difference in the number of landmark and water features. We would not have been able to notice this by the data alone showing the value of considering the spatial context of our results!"}
ggplot() +
...
image: ![](/Users/dwwaweru/Desktop/UMich/Fall21/Geoviz/Advanced/git/EAS501.19/lab1_git_commit.png)
